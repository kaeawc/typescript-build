...
/**
 * Downloads files with a cascading fallback: curl → wget → Node HTTP.
 * The external tools are preferred because they handle retries, redirects,
 * and connection reuse more robustly than a hand-rolled client.
 */
export class DefaultFileDownloader implements FileDownloader {
  public async download(url: string, destination: string): Promise<void> {
    try {
      ...
      console.warn(`[FileDownloader] curl unavailable, falling back to wget: ${this.errorMessage(error)}`);
    }
    ...
    try {
      ...
      console.warn(`[FileDownloader] wget unavailable, falling back to Node HTTP: ${this.errorMessage(error)}`);
    }
  ...
  private async downloadWithNodeHttp(url: string, destination: string, redirectCount: number): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const request = transport.get(
        url,
        { headers: { "User-Agent": "typescript-build" } },
        response => {
