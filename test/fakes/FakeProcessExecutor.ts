...
import type { ExecResult } from "../../src/utils/ExecResult";
...
/**
 * Fake ProcessExecutor for testing. Records calls, matches configured patterns,
 * and returns FakeChildProcess instances for spawn() that tests can drive manually.
 */
export class FakeProcessExecutor implements ProcessExecutor {
  ...
  spawn(command: string, args: string[], options?: SpawnOptions): ChildProcess {
    const process = this.nextSpawnProcess ?? (new FakeChildProcess() as unknown as ChildProcess);
    this.nextSpawnProcess = null;
    const record: { command: string; args: string[]; options?: SpawnOptions; process: ChildProcess } = {
      command,
      args,
      process,
    };
    ...
    this.spawnResponses.push(record);
    return process;
  }
